"use client"
import * as React from "react"
import { WorkspaceLayout, WorkspaceHeader } from "@/components/layout/workspace-layout"
import { FilterBar } from "@/components/ui/filter-bar"
import { DataTable } from "@/components/ui/data-table"
import { ColumnDef } from "@tanstack/react-table"
import { Button } from "@/components/ui/button"
import { Collection } from "@/types"
import { mockCollections } from "@/app/(dashboard)/_data/uco"




export default function ScheduledCollectionsPage() {
  const columns: ColumnDef<Collection>[] = [
    {
      accessorKey: "id",
      header: "Schedule ID",
      cell: ({ row }) => <span className="font-mono text-sm text-primary hover:underline cursor-pointer">{row.getValue("id")}</span>
    },
    {
      accessorKey: "source",
      header: "Source",
    },
    {
      accessorKey: "route",
      header: "Route Assignment",
    },
    {
      accessorKey: "date",
      header: "Scheduled Date",
    },
    {
      accessorKey: "expectedVolume",
      header: () => <div className="text-right">Expected (KG)</div>,
      cell: ({ row }) => <div className="text-right tabular-nums">{row.getValue("expectedVolume")}</div>
    },
  ]

  return (
    <WorkspaceLayout>
      <WorkspaceHeader 
        title="Scheduled Collections" 
        description="Future collections assigned to routes."
        actions={<Button>New Schedule</Button>}
      />
      
      <div className="flex gap-4 border-b border-border mt-2">
        <a href="/uco/collections/pending" className="px-4 py-2 text-sm text-muted-foreground hover:text-foreground">Pending</a>
        <Button variant="ghost" className="rounded-none border-b-2 border-primary rounded-b-sm bg-accent text-accent-foreground font-medium">Scheduled</Button>
        <a href="/uco/collections/completed" className="px-4 py-2 text-sm text-muted-foreground hover:text-foreground">Completed</a>
        <a href="/uco/collections/missed" className="px-4 py-2 text-sm font-medium text-destructive hover:text-destructive/80">Missed Exceptions</a>
      </div>

      <FilterBar 
        placeholder="Search schedules..." 
        views={["Next 7 Days", "Unassigned Routes"]}
      />

      <div className="flex-1 overflow-hidden mt-4">
        <DataTable 
          columns={columns} 
          data={mockCollections} 
        />
      </div>
    </WorkspaceLayout>
  )
}
