...
export default async function NewOrderPage({ searchParams }: { searchParams: { prId?: string } }) {
  ...
  let initialPR = null;
  if (searchParams.prId) {
    initialPR = await db.purchaseRequisition.findUnique({
      where: { id: searchParams.prId, businessId: currentBusinessId },
      include: { lines: true }
    });
  }
