...
import { getCustomer360 } from "@/lib/sales/customer-360";
...
function LinkedList({ title, rows }: { title: string; rows: { code: string; status: string; href: string; extra?: string }[] }) {
  return (
    <Card>
        ...
        {rows.length === 0 ? (
          ...
          rows.map((r) => (
            <div key={r.code} className="flex items-center justify-between text-sm">
              <Link href={r.href} className="font-medium text-primary hover:underline">{r.code}</Link>
...
export default async function Customer360Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const data = await getCustomer360(businessId, id);
  if (!data) notFound();

  const { customer, summary, activity, aging } = data;
  ...
  return (
    <EnterprisePage>
      <EnterprisePageHeader
        title={
          <span className="flex items-center gap-3">
            {customer.name}
            <EnterpriseStatusBadge status={customer.status} showIcon={false} />
            <EnterpriseStatusBadge status={customer.creditStatus} showIcon={false} />
          </span>
        }
        description={`Customer ${customer.code} · since ${fmtDate(customer.since)}`}
        actions={
          <div className="flex items-center gap-2">
            <Link href={`/sales/customers/${id}/statement`} className={buttonVariants({ variant: "outline", size: "sm" })}>
      ...
      <EnterpriseKPIRow className="lg:grid-cols-6">
        <EnterpriseStatCard title="Outstanding" value={formatINR(summary.outstanding)} variant={summary.outstanding > 0 ? "warning" : "default"} />
        ...
        <EnterpriseStatCard title="Collection %" value={summary.collectionRate != null ? `${Math.round(summary.collectionRate * 100)}%` : "—"} />
        <EnterpriseStatCard title="DSO" value={summary.dso != null ? `${summary.dso}d` : "—"} />
        <EnterpriseStatCard title="Revenue (YTD)" value={formatINR(summary.revenueYtd)} />
      </EnterpriseKPIRow>

      {/* Sales activity */}
      <EnterpriseKPIRow className="lg:grid-cols-5">
        <EnterpriseStatCard title="Quotations" value={activity.quotations} />
        <EnterpriseStatCard title="Orders" value={activity.orders} />
        <EnterpriseStatCard title="Shipments" value={activity.shipments} />
        <EnterpriseStatCard title="Invoices" value={activity.invoices} />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader className="pb-2"><CardTitle className="text-base font-medium">Ledger snapshot</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base font-medium">Aging snapshot</CardTitle></CardHeader>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        <LinkedList title="Open orders" rows={data.openOrders.map((o) => ({ code: o.code, status: o.status, href: o.href, extra: formatINR(o.total) }))} />
        <LinkedList title="Pending shipments" rows={data.pendingShipments} />
        <LinkedList title="Recent deliveries" rows={data.recentDeliveries} />
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base font-medium">Top products</CardTitle></CardHeader>
          <CardContent className="space-y-2">
      ...
      {/* Timeline */}
      <EnterpriseSection title="Timeline" description="Every event for this customer, newest first">
