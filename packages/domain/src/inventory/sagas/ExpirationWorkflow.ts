...
export class ExpirationWorkflow {
  constructor(private readonly repository: AggregateRepository) {}

  public async expireReservation(reservationId: string, actorId: string, allocatedLocations: Array<{locationId: string, productId: string, quantity: any}>): Promise<void> {
    const reservation = await this.repository.load(reservationId, 'Reservation', () => new ReservationAggregate(reservationId));
    reservation.expireReservation(actorId);
    await this.repository.save(reservation);

    // Release bucket allocations
